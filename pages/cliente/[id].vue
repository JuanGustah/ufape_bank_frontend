...
function salvar() {
  let clienteObject = toRaw(cliente);
  let id = route.params.id;

  ClienteService.atualizar(id, clienteObject).then((resposta) => {
    navigateTo("/cliente");
  });
}

function buscarCliente(id) {
  ClienteService.buscar(id).then((resposta) => {
    Object.assign(cliente, resposta.data);
  });
}

onMounted(() => {
  buscarCliente(route.params.id);
});
</script>
