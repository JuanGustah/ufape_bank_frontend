<template>
  <Container>
    <h2 style="color: white">Nova Conta</h2>
    <form v-on:submit.prevent="salvar">
      ...
      <label for="conta-tipo">Tipo conta</label>
      <select v-model:="tipoConta" id="conta-tipo">
        <option value="" disabled selected>Selecione um tipo de conta</option>
        <option value="1">Conta Padrão</option>
        <option value="2">Conta Poupança</option>
        <option value="3">Conta Bonificada</option>
        <option value="4">Conta com imposto</option>
      </select>
...
function salvar() {
  ...
  let tipoContaSelecionado = tipoConta.value;

  ContaService.criar(tipoContaSelecionado, contaObject).then(() => {
    navigateTo("/conta");
  });
}
